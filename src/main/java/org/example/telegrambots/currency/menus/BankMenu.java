...
public class BankMenu implements TelegramMenu {
    ...
    public InlineKeyboardMarkup createMenu(UserMessage userMessage) {
        ...
        InlineKeyboardButton buttonPrivatbank = new InlineKeyboardButton(
            (Bank.PRIVATBANK.equals(bank) ? markEmoji : "") + userMessage.getUser()
                .getTranslation(Commands.BANK_PRIVATBANK.getButtonText()));
        ...
        InlineKeyboardButton buttonMonobank = new InlineKeyboardButton(
            (Bank.MONOBANK.equals(bank) ? markEmoji : "") + userMessage.getUser()
                .getTranslation(Commands.BANK_MONOBANK.getButtonText()));
        ...
        InlineKeyboardButton buttonOschad = new InlineKeyboardButton(
            (Bank.OSCHADBANK.equals(bank) ? markEmoji : "")
                + userMessage.getUser().getTranslation(Commands.BANK_OSCHADBANK.getButtonText()));
        ...
        InlineKeyboardButton buttonNbu = new InlineKeyboardButton(
            (Bank.NBU.equals(bank) ? markEmoji : "") + userMessage.getUser()
                .getTranslation(Commands.BANK_NBU.getButtonText()));
        buttonNbu.setCallbackData(Commands.BANK_NBU.toString());
        row4.add(buttonNbu);

        List<InlineKeyboardButton> row5 = new ArrayList<>();
        InlineKeyboardButton buttonBack = new InlineKeyboardButton(
            userMessage.getUser().getTranslation(Commands.BACK.getButtonText()));
